...
module.exports = function (opts) {
	...
	return through.obj(function (file, enc, cb) {
		...
		Promise.resolve()
			...
			.then(function (projLocation) {
				packageJson = require(projLocation);
				var packageJsonDir = path.dirname(projLocation);
				if (!!packageJson.jspm.configFile) { // explicit location
					configJsLoc = path.resolve(packageJsonDir, packageJson.jspm.configFile);
				}
				else if (!!packageJson.jspm.directories.baseURL) { // baseURL based
					configJsLoc = path.resolve(packageJsonDir, packageJson.jspm.directories.baseURL+'/config.js');
				}
				else
